...
def compute_pca_core_features(hidden_states, variance_threshold=0.99):
    ...
    for layer_idx in layer_indices:
        ...
        # Free the original tensor reference for this layer if possible
        # (we can't delete from dict while iterating, but we process immediately)
        ...
        try:
            # Full SVD - no truncation as requested
...
def plot_pca_core_features(results, model_names, output_dir, variance_threshold=0.99):
    ...
    ax.set_title(f"Intrinsic Dimensionality: Principal Components for {variance_threshold*100:.0f}% Variance", fontsize=16)
...
class AttentionWeightCapture:
    ...
    @staticmethod
    def compute_attention_weights(model, x, layer_idx, dtype):
        ...
        if getattr(block, "norm_pos", None) in {"before", "both"}:
            x_normed = block.attn_norm(x)
        else:
            x_normed = x

        if getattr(attn, "gated_attention_enabled", False):
            qkv_g = attn.c_attn(x_normed)
            q, k, v, g = qkv_g.split(attn.n_embd, dim=2)
            del qkv_g
        else:
            qkv = attn.c_attn(x_normed)
        ...
        if g is not None:
            g = g.view(B, T, attn.n_head, attn.head_dim).transpose(1, 2)
        ...
        if hasattr(attn, "rope"):
            q, k = attn.rope(q, k, pos_offset=0)

        if hasattr(attn, "rope") and getattr(attn.rope, "yarn_enabled", False) and hasattr(attn.rope, "attn_scale"):
            scale = float(attn.rope.attn_scale)
        else:
        ...
        return attn_weights, v, g


def compute_all_attention_weights(model, input_ids, dtype):
    """Compute attention weights and hidden states with immediate CPU transfer."""
    model.eval()
    attention_weights = {}
    hidden_states = {}
    ...
    with torch.no_grad():
        ...
        x = model.transformer.wte(input_ids).to(dtype)

        if getattr(model.config, "smear_gate_enabled", False) and T > 1 and hasattr(model, "smear_gate"):
            gate = torch.sigmoid(model.smear_gate(x[:, 1:, : model.config.smear_gate_dim]))
            gate = model.smear_lambda * gate
        ...
        hidden_states[0] = x.clone().cpu()

        q_anc = k_anc = v_anc = g_anc = None
        
        anchor_enabled = getattr(model.config, "anchor_enabled", False) or getattr(model.config, "decouple_anchor", False)
        
        if anchor_enabled and hasattr(model, "anchor"):
            emb = model.transformer.wte(input_ids).to(dtype)
            q_anc, k_anc, v_anc, g_anc = model.anchor(emb)
            del emb
        elif getattr(model.config, "decouple_anchor", False) and hasattr(model, "_compute_decoupled_anchors"):
            emb = model.transformer.wte(input_ids).to(dtype)
            q_anc, k_anc, v_anc, g_anc = model._compute_decoupled_anchors(emb)
            del emb

        q_1 = k_1 = v_1 = g_1 = None

        for layer_idx, block in enumerate(model.transformer.layers):
            attn_w, v, g = AttentionWeightCapture.compute_attention_weights(model, x, layer_idx, dtype=dtype)
            
            # Move to CPU immediately
            attention_weights[layer_idx] = attn_w.cpu()
            del attn_w, v, g
            ...
            try:
                ...
                block_out = block(x, past_kv=None, use_cache=False)

            if isinstance(block_out, tuple):
                if len(block_out) == 5:
                    x, q_1, k_1, v_1, g_1 = block_out
                elif len(block_out) == 6:
                    x, _, q_anc, k_anc, v_anc, g_anc = block_out
                else:
                    x = block_out[0]
            else:
                x = block_out
            ...
            hidden_states[layer_idx + 1] = x.clone().cpu()
...
def compute_first_token_attention_score(attention_weights):
    """Compute first token attention scores from CPU tensors."""
    first_token_scores = {}
...
def compute_token_importance(attention_weights):
    """Compute token importance from CPU tensors."""
    token_importance = {}
    for layer_idx, attn in attention_weights.items():
...
def compute_attention_entropy(attention_weights):
    """Compute attention entropy from CPU tensors."""
    entropies = {}
    eps = 1e-10
    for layer_idx, attn in attention_weights.items():
...
def compute_massive_activations(hidden_states):
    """Compute max activations from CPU tensors."""
    max_activations = {}
...
def compute_hidden_state_norms(hidden_states):
    """Compute hidden state norms from CPU tensors."""
    norms = {}
    for layer_idx, h in hidden_states.items():
        if layer_idx == "final":
            continue
...
def compute_hidden_state_similarity(hidden_states):
    """Compute consecutive layer similarity from CPU tensors."""
    similarities = {}
    layer_indices = sorted([k for k in hidden_states.keys() if k != "final"])
...
def compute_first_layer_similarity(hidden_states):
    """Compute similarity to first layer from CPU tensors."""
    similarities = {}
    layer_indices = sorted([k for k in hidden_states.keys() if k != "final"])
...
def compute_token_to_token_similarity(hidden_states):
    """Compute token-to-token similarity from CPU tensors."""
    similarities = {}
    for layer_idx, h in hidden_states.items():
        if layer_idx == "final":
            continue
...
def compute_token_to_token_similarity_matrices(hidden_states):
    """Compute token similarity matrices from CPU tensors."""
    similarity_matrices = {}
    for layer_idx, h in hidden_states.items():
        if layer_idx == "final":
            continue
...
def compute_token_to_token_similarity_std(hidden_states):
    """Compute token similarity std from CPU tensors."""
    similarity_stds = {}
    for layer_idx, h in hidden_states.items():
        if layer_idx == "final":
            continue
...
def compute_top_k_token_importance(attention_weights, k_values=(1, 2, 3, 10)):
    """Compute top-k importance from CPU tensors."""
...
def analyze_gating_scores(model, input_ids, dtype):
    """Analyze gating scores with memory optimization."""
    if not getattr(model.config, "gated_attention_enabled", False):
        return None
    ...
    with torch.no_grad():
        ...
        for layer_idx, block in enumerate(model.transformer.layers):
            ...
            if not getattr(attn, "gated_attention_enabled", False):
                gating_stats[layer_idx] = {
                    "mean": np.nan,
                    "std": np.nan,
                    "min": np.nan,
                    "max": np.nan,
                ...
                qkv_g = attn.c_attn(x_normed)
                _, _, _, g = qkv_g.split(attn.n_embd, dim=2)
                del qkv_g
                g = g.view(B, T, attn.n_head, attn.head_dim).transpose(1, 2)

                gate_scores = torch.sigmoid(g.float())
                gating_stats[layer_idx] = {
                    "mean": gate_scores.mean().item(),
                    "std": gate_scores.std().item(),
                    "min": gate_scores.min().item(),
                    "max": gate_scores.max().item(),
                    "sparsity": (gate_scores < 0.2).float().mean().item(),
                }
                del g, gate_scores
            ...
            try:
                if anchor_enabled:
                    block_out = block(x, q_ref=q_anc, k_ref=k_anc, v_ref=v_anc, g_ref=g_anc)
                else:
                    block_out = block(x, v_1=v_1, q_1=q_1, k_1=k_1, g_1=g_1)
            except TypeError:
                block_out = block(x)
...
def analyze_value_residual_lambdas(model):
    """Analyze lambda parameters - already memory efficient."""
    lambda_stats = {}
...
def plot_attention_analysis(results, model_names, output_dir):
    ...
    has_gating = any(
        results[name].get('gating_stats') is not None
        for name in model_names if name in results
    )

    if has_gating:
        metrics = {
            "mean": {
                "title": "Mean Gating Score per Layer",
                "ylabel": "Mean Gate Score",
                "filename": "gating_mean.png"
            },
            "std": {
                "title": "Std of Gating Score per Layer",
                "ylabel": "Std Gate Score",
                "filename": "gating_std.png"
            },
            "sparsity": {
                "title": "Gating Sparsity (% < 0.2) per Layer",
                "ylabel": "Sparsity",
                "filename": "gating_sparsity.png"
            }
        }
        ...
        for metric, meta in metrics.items():
            ...
            for i, name in enumerate(model_names):
                if name in results and results[name].get('gating_stats') is not None:
                    display_name = format_display_name(name)
                    gating_stats = results[name]['gating_stats']
                    layers = sorted(gating_stats.keys())
                    values = [gating_stats[l][metric] for l in layers]
            ...
            ax.set_xlabel("Layer Index", fontsize=25)
            ax.set_ylabel(meta["ylabel"], fontsize=25)
            ax.legend(fontsize=20)
            ax.grid(True, alpha=0.3)

            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, meta["filename"]), dpi=150)
            plt.close()
        ...
        for i, name in enumerate(model_names):
            if name in results and results[name].get('gating_stats') is not None:
                ...
                mins = [gating_stats[l]['min'] for l in layers]
                maxs = [gating_stats[l]['max'] for l in layers]
                means = [gating_stats[l]['mean'] for l in layers]
        ...
        ax.set_xlabel("Layer Index", fontsize=12)
        ax.set_ylabel("Gate Score", fontsize=12)
        ax.set_title("Gating Score Range per Layer", fontsize=14)
        ax.legend(fontsize=20)
        ax.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "gating_range.png"), dpi=150)
        plt.close()
    ...
    for name in model_names:
        if name in results and "token_similarity_matrices" in results[name]:
            ...
            # Clear the matrices after plotting to free memory
...
def print_summary(results, model_names):
    ...
    for name in model_names:
        ...
        if results[name].get("gating_stats"):
            gating = results[name]["gating_stats"]
            means = [g["mean"] for g in gating.values() if isinstance(g["mean"], float) and not np.isnan(g["mean"])]
            spars = [g["sparsity"] for g in gating.values() if isinstance(g["sparsity"], float) and not np.isnan(g["sparsity"])]
            if means:
                print("\n📊 Gating Analysis:")
                print(f"   Mean Gating Score: {float(np.mean(means)):.4f}")
            if spars:
                print(f"   Mean Sparsity:     {float(np.mean(spars)):.2%}")
    ...
    if len(model_names) > 1:
        ...
        if len(analyzed_models) >= 2:
            ...
            for name in analyzed_models:
                if results[name].get("gating_stats"):
                    vals = [g["sparsity"] for g in results[name]["gating_stats"].values() if isinstance(g["sparsity"], float) and not np.isnan(g["sparsity"])]
                    if vals:
                        gating_all[name] = float(np.mean(vals))
            if gating_all:
                row = f"{'Gating Sparsity (mean)':<30}"
                for name in analyzed_models:
                    if name in gating_all:
                        row += f" {gating_all[name]:>11.1%}"
                    else:
                        row += f" {'N/A':>12}"
                print(row)
...
def load_model_from_checkpoint(ckpt_path, device):
    ...
    # Check for anchor before clearing checkpoint
...
def analyze_single_model(name, ckpt_path, input_ids, analysis_dtype, output_dir, pca_variance_threshold, data_source):
    """Analyze a single model with aggressive memory cleanup."""
    display_name = format_display_name(name)
    print(f"\n{'=' * 60}")
    ...
    try:
        ...
        print("Computing attention weights and hidden states...")
        attention_weights, hidden_states = compute_all_attention_weights(
        ...
        # Clear attention weights after computing all attention-based metrics
        # But keep a small subset for visualization
        ...
        # Clear hidden states after all metrics computed
        del hidden_states
        gc.collect()
        cleanup_memory()

        gating_stats = None
        if getattr(config, "gated_attention_enabled", False):
            print("Analyzing gating scores...")
            gating_stats = analyze_gating_scores(model, input_ids, dtype=analysis_dtype)
            cleanup_memory()
        ...
        # Delete model before storing results
        del model
        cleanup_memory()
...
Examples:
  ...
  python aa.py --use-random-data
    ...
    # Prepare input data once (shared across all models)
    val_loader = None
    ...
    # Process each model one at a time with full cleanup between
    for name, ckpt_path in models_to_analyze.items():
        # Generate input if not using validation data (need vocab size from first model)
        if input_ids is None:
            # Load just the config to get vocab size
            checkpoint = torch.load(ckpt_path, map_location='cpu', weights_only=False)
    ...
    # Clear input_ids after all models processed
    del input_ids
    cleanup_memory()
    ...
    with open(results_file, "w") as f:
        ...
        for name in model_names:
            ...
            if results[name].get("gating_stats"):
                f.write("\n" + "-" * 40 + "\n")
                f.write("Gating Statistics:\n")
                f.write("-" * 40 + "\n")
                for layer, stats in sorted(results[name]["gating_stats"].items()):
                    f.write(
                        f"  Layer {layer:2d}: mean={stats['mean']:.4f}, "
                        f"std={stats['std']:.4f}, sparsity={stats['sparsity']:.2%}\n"
                    )
