...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        if output_attentions:
            ...
            causal_mask = torch.tril(torch.ones(T, k.size(-2), device=q.device)).view(1, 1, T, k.size(-2))
        ...
        if self.gated_attention_enabled:
            attention_output, gate_scores = self._apply_gating(attention_output, g, g_anc, dynamic_scales)
            if output_gating_scores:
                gate_scores_return = gate_scores
        ...
        def pack_return(ret_tuple):
            ...
            if output_gating_scores:
                ret_tuple = ret_tuple + (gate_scores_return,)
            return ret_tuple

        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                return pack_return((x, (k, v), q_residual, k_residual, v_residual, g_residual, anchor_deltas))
            else:
                return pack_return((x, q_residual, k_residual, v_residual, g_residual, anchor_deltas))
        else:
            if use_cache:
                return pack_return((x, (k, v), anchor_deltas))
            else:
                return pack_return((x, anchor_deltas))
...
class Block(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                x, new_kv, q_res, k_res, v_res, g_res, anchor_deltas = attn_content
            else:
                x, q_res, k_res, v_res, g_res, anchor_deltas = attn_content
                new_kv = None
        else:
            if use_cache:
                x, new_kv, anchor_deltas = attn_content
            else:
                x, anchor_deltas = attn_content
                new_kv = None
        ...
        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                res = (x, new_kv, q_res, k_res, v_res, g_res, anchor_deltas)
            else:
                res = (x, q_res, k_res, v_res, g_res, anchor_deltas)
        else:
            if use_cache:
                res = (x, new_kv, anchor_deltas)
            else:
                res = (x, anchor_deltas)
    ...
    def forward(self, idx, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        for layer_idx, block in enumerate(self.transformer.layers):
            ...
            if layer_idx == 0 and not self.config.decouple_anchor:
                if use_cache:
                    x, present_kv, q_anc, k_anc, v_anc, g_anc, anchor_deltas = block_content
                    new_kv.append(present_kv)
                else:
                    x, q_anc, k_anc, v_anc, g_anc, anchor_deltas = block_content
            else:
                if use_cache:
                    x, present_kv, anchor_deltas = block_content
                    new_kv.append(present_kv)
                else:
                    x, anchor_deltas = block_content
