...
class DynamicMixingModule(nn.Module):
    """
    Dynamic mixing module that produces per-position scaling factors for lambda parameters.
    
    Input: x of shape [B, T, n_embd]
    Output: scaling factors of shape [B, T, output_dim]
    
    Architecture: Linear(n_embd, 16) -> GELU -> Linear(16, output_dim) -> Sigmoid
    The output layer is zero-initialized so that initial sigmoid output is 0.5.
    Lambda init values should be doubled to compensate (0.5 * 2 = 1.0 effective multiplier at init).
    """
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        x = self.fc2(x)
        x = torch.sigmoid(x)  # [B, T, output_dim]
        return x
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx=0):
        ...
        # NEW: Dynamic mixing and anchor delta toggles
        self.dynamic_mixing_enabled = config.dynamic_mixing_enabled
        self.anchor_delta_enabled = config.anchor_delta_enabled
        ...
        # QK normalization for attention computation
        ...
        # Compute lambda init multiplier (2x if dynamic mixing enabled to compensate for sigmoid)
        ...
        # NEW: Dynamic mixing module
        if self.dynamic_mixing_enabled and should_have_residual_params:
            # Base: 8 outputs for Q,K,V,G x lambda1,lambda2
            # If anchor_delta_enabled: +4 for delta_q, delta_k, delta_v, delta_g
            num_lambda_outputs = 8 if self.gated_attention_enabled else 6
            num_delta_outputs = 4 if (self.anchor_delta_enabled and self.gated_attention_enabled) else (3 if self.anchor_delta_enabled else 0)
            output_dim = num_lambda_outputs + num_delta_outputs
        ...
        # NEW: Anchor delta parameters
        if self.anchor_delta_enabled:
            # Delta lambda init multiplier (2x if dynamic mixing to compensate for sigmoid)
            delta_init_multiplier = 2.0 if self.dynamic_mixing_enabled else 1.0
            delta_init = config.anchor_delta_init * delta_init_multiplier
            
            if self.residual_mode == "elementwise":
                self.delta_lambda_q = nn.Parameter(torch.full((self.n_head, self.head_dim), delta_init))
                self.delta_lambda_k = nn.Parameter(torch.full((self.n_head, self.head_dim), delta_init))
                self.delta_lambda_v = nn.Parameter(torch.full((self.n_head, self.head_dim), delta_init))
                if self.gated_attention_enabled:
                    self.delta_lambda_g = nn.Parameter(torch.full((self.n_head, self.head_dim), delta_init))
            elif self.residual_mode == "headwise":
                self.delta_lambda_q = nn.Parameter(torch.full((self.n_head,), delta_init))
                self.delta_lambda_k = nn.Parameter(torch.full((self.n_head,), delta_init))
                self.delta_lambda_v = nn.Parameter(torch.full((self.n_head,), delta_init))
                if self.gated_attention_enabled:
                    self.delta_lambda_g = nn.Parameter(torch.full((self.n_head,), delta_init))
            else:  # scalar
                self.delta_lambda_q = nn.Parameter(torch.tensor(delta_init))
                self.delta_lambda_k = nn.Parameter(torch.tensor(delta_init))
                self.delta_lambda_v = nn.Parameter(torch.tensor(delta_init))
                if self.gated_attention_enabled:
                    self.delta_lambda_g = nn.Parameter(torch.tensor(delta_init))
    ...
    def _get_lambda_views(self, lambda1, lambda2, dtype):
        """Get properly shaped lambda views based on residual_mode."""
        if self.residual_mode == "elementwise":
    ...
    def _get_delta_lambda_view(self, delta_lambda, dtype):
        """Get properly shaped delta lambda view based on residual_mode."""
        if self.residual_mode == "elementwise":
            return delta_lambda.view(1, self.n_head, 1, self.head_dim).to(dtype)
        elif self.residual_mode == "headwise":
            return delta_lambda.view(1, self.n_head, 1, 1).to(dtype)
        else:  # scalar
            return delta_lambda.to(dtype)
    ...
    def _scaled_dot_product_attention(self, q, k, v, attn_mask=None, dropout_p=0.0, is_causal=True, 
        ...
        # Document-level masking with flash_attn_varlen_func
    ...
    def _apply_gating(self, attention_output, g, g_anc, dynamic_scales=None):
        ...
        # Apply gate residual mixing with anchor
        should_mix_gate = (
        ...
        if should_mix_gate:
            ...
            if dynamic_scales is not None and self.gated_attention_enabled:
                scale_g1 = dynamic_scales['g1']  # [B, 1, T, 1]
                scale_g2 = dynamic_scales['g2']  # [B, 1, T, 1]
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        # Compute dynamic mixing scales if enabled
        dynamic_scales = None
        if self.dynamic_mixing_enabled and (self.decouple_anchor or self.layer_idx > 0):
            if hasattr(self, 'dynamic_mixing'):
                scales = self.dynamic_mixing(x)  # [B, T, output_dim]
                # Reshape scales to [B, 1, T, 1] for broadcasting
                dynamic_scales = {}
                idx = 0
                ...
                if self.gated_attention_enabled:
                    ...
                    idx += 1
                if self.anchor_delta_enabled:
                    dynamic_scales['delta_q'] = scales[:, :, idx].view(B, 1, T, 1)
                    idx += 1
                    dynamic_scales['delta_k'] = scales[:, :, idx].view(B, 1, T, 1)
                    idx += 1
                    dynamic_scales['delta_v'] = scales[:, :, idx].view(B, 1, T, 1)
                    idx += 1
                    if self.gated_attention_enabled:
                        dynamic_scales['delta_g'] = scales[:, :, idx].view(B, 1, T, 1)
        ...
        # Store raw values for anchor delta update (before residual mixing)
        q_raw = q.clone() if self.anchor_delta_enabled else None
        k_raw = k.clone() if self.anchor_delta_enabled else None
        v_raw = v.clone() if self.anchor_delta_enabled else None
        g_raw = g.clone() if (self.anchor_delta_enabled and g is not None) else None
        ...
        # Prepare anchor delta outputs if enabled
        anchor_deltas = None
        if self.anchor_delta_enabled:
            anchor_deltas = {
                'q': q_raw,
                ...
                'v': v_raw,
                'g': g_raw,
                'scales': dynamic_scales  # Pass scales for delta computation in OBPM
            }
        
        # Return format depends on whether we're layer 0 with non-decoupled anchors
        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                return x, (k, v), q_residual, k_residual, v_residual, g_residual, anchor_deltas
            else:
                return x, q_residual, k_residual, v_residual, g_residual, anchor_deltas
        else:
            if use_cache:
                return x, (k, v), anchor_deltas
            else:
                return x, anchor_deltas
...
class Block(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        # Parse attention output based on layer and config
        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                x, new_kv, q_res, k_res, v_res, g_res, anchor_deltas = attn_out
            else:
                x, q_res, k_res, v_res, g_res, anchor_deltas = attn_out
                new_kv = None
        else:
            if use_cache:
                x, new_kv, anchor_deltas = attn_out
            else:
                x, anchor_deltas = attn_out
                new_kv = None
        ...
        if self.layer_idx == 0 and not self.decouple_anchor:
            if use_cache:
                return x, new_kv, q_res, k_res, v_res, g_res, anchor_deltas
            else:
                return x, q_res, k_res, v_res, g_res, anchor_deltas
        else:
            if use_cache:
                return x, new_kv, anchor_deltas
            else:
                return x, anchor_deltas
...
class OBPM(nn.Module):
    """
    Unified ResGated Transformer with configurable anchor decoupling and residual normalization.
    
    Configuration options:
    - decouple_anchor: When True, creates separate anchor projections from embeddings that all 
      layers use. When False, uses layer 0's Q,K,V,G as residuals for subsequent layers.
    
    - q_residual_norm_enabled: When True, applies RMSNorm (via QKNorm) to Q residual sources.
    - k_residual_norm_enabled: When True, applies RMSNorm (via QKNorm) to K residual sources.
    - v_residual_norm_enabled: When True, applies RMSNorm to V residual sources.
    - g_residual_norm_enabled: When True, applies RMSNorm to G residual sources.
    
    - embedding_layer0_mix_enabled: When True, the input to layer 1 is a learnable mixture
      of layer 0's output and the input embeddings: alpha1 * layer0_output + alpha2 * embeddings
    
    - dynamic_mixing_enabled: When True, uses a small MLP to produce per-position scaling 
      factors for the lambda parameters. The MLP is: Linear(n_embd, 16) -> GELU -> 
      Linear(16, 8 or 12) -> Sigmoid. Lambda init values are doubled to compensate for 
      sigmoid(0)=0.5.
    
    - anchor_delta_enabled: When True, anchors are updated after each layer:
      anchor_s = anchor_s + delta_lambda_s * current_layer_s
      This allows anchors to accumulate information through the layers.
    
    Example configurations:
    1. decouple_anchor=False, all norms=False: 
       Layer 0's raw Q,K,V,G are used as residuals for subsequent layers
    
    2. decouple_anchor=False, all norms=True:
       Layer 0's normalized Q,K,V,G are used as residuals
    
    3. decouple_anchor=False, v_residual_norm_enabled=True, others=False:
       Only V residuals are normalized (matching original ResFormer behavior)
    
    4. decouple_anchor=True, selective norms:
       Separate anchor projections from embeddings with selective normalization
    
    5. embedding_layer0_mix_enabled=True:
       Learnable mixture between embeddings and layer 0 output for layer 1 input
    
    6. dynamic_mixing_enabled=True:
       Per-position learned scaling of lambda parameters
    
    7. anchor_delta_enabled=True:
       Anchors accumulate information from each layer
    """
    ...
    def __init__(self, config: ModelConfig):
        ...
        # Decoupled anchors: separate projection from embeddings to Q, K, V, G
        if config.decouple_anchor:
            ...
            # RMSNorm for each anchor component (conditionally based on individual toggles)
        ...
        # NEW: Learnable mixture between embeddings and layer 0 output
    ...
    def _init_weights(self, module, std=0.02, init_cutoff_factor=None):
        ...
        if isinstance(module, DynamicMixingModule):
            # Don't override the zero-init for fc2
            return
    ...
    def _compute_decoupled_anchors(self, embeddings):
        """Compute decoupled Q, K, V, G anchors from embeddings with optional RMSNorm."""
        B, T, C = embeddings.size()
        n_head = self.config.n_head
        head_dim = C // n_head
        
        # Project embeddings to Q, K, V, (G)
        ...
        # Apply RMSNorm to each anchor based on individual toggles
    ...
    def _apply_anchor_delta(self, q_anc, k_anc, v_anc, g_anc, anchor_deltas, block):
        """Apply anchor delta update: anchor_s = anchor_s + delta_lambda_s * current_layer_s"""
        if anchor_deltas is None or not self.config.anchor_delta_enabled:
            return q_anc, k_anc, v_anc, g_anc
        
        attn = block.attn
        dtype = q_anc.dtype if q_anc is not None else torch.float32
        
        # Get delta lambdas with proper shapes
        delta_q = attn._get_delta_lambda_view(attn.delta_lambda_q, dtype)
        delta_k = attn._get_delta_lambda_view(attn.delta_lambda_k, dtype)
        delta_v = attn._get_delta_lambda_view(attn.delta_lambda_v, dtype)
        
        # Apply dynamic scaling to deltas if enabled
        if anchor_deltas.get('scales') is not None and self.config.dynamic_mixing_enabled:
            scales = anchor_deltas['scales']
            if 'delta_q' in scales:
                delta_q = delta_q * scales['delta_q']
            if 'delta_k' in scales:
                delta_k = delta_k * scales['delta_k']
            if 'delta_v' in scales:
                delta_v = delta_v * scales['delta_v']
        
        # Update anchors
        if q_anc is not None and anchor_deltas['q'] is not None:
            q_anc = q_anc + delta_q * anchor_deltas['q']
        if k_anc is not None and anchor_deltas['k'] is not None:
            k_anc = k_anc + delta_k * anchor_deltas['k']
        if v_anc is not None and anchor_deltas['v'] is not None:
            v_anc = v_anc + delta_v * anchor_deltas['v']
        
        # Handle G anchor
        if g_anc is not None and anchor_deltas['g'] is not None and self.config.gated_attention_enabled:
            delta_g = attn._get_delta_lambda_view(attn.delta_lambda_g, dtype)
            if anchor_deltas.get('scales') is not None and self.config.dynamic_mixing_enabled:
                if 'delta_g' in anchor_deltas['scales']:
                    delta_g = delta_g * anchor_deltas['scales']['delta_g']
            g_anc = g_anc + delta_g * anchor_deltas['g']
    ...
    def forward(self, idx, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, window_size=None):
        ...
        # Compute anchors based on configuration
        if self.config.decouple_anchor:
            # Decoupled: compute anchors from embeddings (before smear gate and dropout)
            q_anc, k_anc, v_anc, g_anc = self._compute_decoupled_anchors(x)
        else:
            # Not decoupled: anchors will come from layer 0
        ...
        # Save embeddings for optional mixture with layer 0 output
        ...
        for layer_idx, block in enumerate(self.transformer.layers):
            ...
            # Parse block output based on layer and config
            if layer_idx == 0 and not self.config.decouple_anchor:
                # Layer 0 returns residuals for subsequent layers
                if use_cache:
                    x, present_kv, q_anc, k_anc, v_anc, g_anc, anchor_deltas = block_out
                    new_kv.append(present_kv)
                else:
                    x, q_anc, k_anc, v_anc, g_anc, anchor_deltas = block_out
            else:
                if use_cache:
                    x, present_kv, anchor_deltas = block_out
                    new_kv.append(present_kv)
                else:
                    x, anchor_deltas = block_out
            
            # NEW: Apply anchor delta update after each layer
            if self.config.anchor_delta_enabled and anchor_deltas is not None:
                q_anc, k_anc, v_anc, g_anc = self._apply_anchor_delta(
                    q_anc, k_anc, v_anc, g_anc, anchor_deltas, block
                )
            
            # NEW: Apply embedding-layer0 mixture after layer 0
