...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx=0):
        ...
        # Only create residual parameters for layers > 0
        if layer_idx > 0:
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        # For layer 0, save raw reshaped values as residuals for subsequent layers
        # These are saved BEFORE any normalization, RoPE, or cache operations
        if self.layer_idx == 0:
            q_residual = q
            k_residual = k
            v_residual = v
            g_residual = g  # Raw gate values for subsequent layers
        
        # Apply Q/K/V residual mixing (only for layer > 0)
        if self.layer_idx > 0:
        ...
        g_out = None
        if self.gated_attention_enabled:
            attention_output, g_out = self._apply_gating(attention_output, g, g_1)
        
        x = self.c_proj(attention_output)
        
        # Return based on layer index and use_cache
        # Layer 0 returns residuals (pre-norm, pre-RoPE, pre-cache) for subsequent layers
        # Other layers return minimal output
        if self.layer_idx == 0:
            # Return the clean residuals captured before any transformations
            if use_cache:
...
class Block(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        # Attention forward - returns different outputs based on layer_idx
        attn_out = self.attn(
            x,
            past_kv=past_kv,
            use_cache=use_cache,
        ...
        # Unpack based on layer index
        if self.layer_idx == 0:
            if use_cache:
                x, new_kv, q_1_out, k_1_out, v_1_out, g_1_out = attn_out
            else:
                x, q_1_out, k_1_out, v_1_out, g_1_out = attn_out
                new_kv = None
        else:
            if use_cache:
                x, new_kv = attn_out
        ...
        # Return based on layer index
        if self.layer_idx == 0:
            if use_cache:
                return x, new_kv, q_1_out, k_1_out, v_1_out, g_1_out
            else:
                return x, q_1_out, k_1_out, v_1_out, g_1_out
        else:
            if use_cache:
                return x, new_kv
            else:
...
class OBPM(nn.Module):
    ...
    def forward(self, idx, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, window_size=None):
        ...
        for layer_idx, block in enumerate(self.transformer.layers):
            ...
            if layer_idx == 0:
                if use_cache:
                    x, present_kv, q_1, k_1, v_1, g_1 = block_out
