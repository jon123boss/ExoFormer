...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, cu_doc_len=None, max_doc_len=None, 
        ...
        # QK normalization (applied to q, k used in attention)
        if self.q_norm is not None:
            q = self.q_norm(q)
            k = self.k_norm(k)
        
        # For layer 0, capture residuals after normalization but before RoPE/cache
        # v_norm and g_norm are only applied to residuals, not to attention/gating computation
        if self.layer_idx == 0:
            q_residual = q
            k_residual = k
        ...
        # Layer 0 returns residuals (post-norm, pre-RoPE, pre-cache) for subsequent layers
